...
public class exportContactsActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		m_ExportContactsToLocalThread = new Thread() {
			...
			public void run() {
				try {
					if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {// 判断当前手机终端是否有SD卡
						myOperator.exportContactOneByOneAsVcardFileToSD(
								Constant.SDLOCALPATH, contactPos);
						path = Constant.SDLOCALPATH;
					} else {
						myOperator.exportContactOneByOneAsVcardFileToSD(
								Constant.LOCALPATH, contactPos);
						path = Constant.LOCALPATH;
					}
	...
	public void onexportedbtn(View view) {
		if (exportsuccess) {
			...
			intent.setClass(exportContactsActivity.this, RainbowContactsActivity.class);
